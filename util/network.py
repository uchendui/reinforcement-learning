...
class QNetworkBuilder(NetworkBuilder):
    def __init__(self, input_dim, output_dim, layers=(512,), activations=(tf.nn.relu, None), scope='q_network',
                 conv=False):
        super(QNetworkBuilder, self).__init__(input_dim, output_dim, layers, activations, scope, conv)
...
class ReinforceNetworkBuilder(NetworkBuilder):
    def __init__(self, input_dim, output_dim, layers=(512,), activations=(tf.nn.relu, None), scope='q_network',
                 conv=False):
        super(ReinforceNetworkBuilder, self).__init__(input_dim, output_dim, layers, activations, scope, conv)

        # Create variables
        self.input_ph, self.output_pred = self.create_network()
        self.q_values_ph = tf.placeholder(dtype=tf.float32, shape=[None])
        self.actions_ph = tf.placeholder(dtype=tf.float32, shape=[None, self.output_dim])
